...
**Necessidades e Objetivos**
Camila deseja reportar um problema de infraestrutura de forma simples e rápida. Quer ter certeza de que seu chamado foi registrado e não será ignorado, e por isso valoriza a possibilidade de acompanhar o status da solicitação (aberto, em andamento, resolvido) de maneira transparente e acessível. O sistema requer login com matrícula e senha; caso não tenha acesso no momento, ela pode se dirigir à portaria, onde um funcionário registra o chamado em seu nome.
...
| **Necessidades** | Reportar um problema pelo celular em menos de 1 minuto, usando apenas a matrícula. Acompanhar o status do chamado para saber se foi registrado e está sendo tratado. |

---

### Roberta Lima Costa – Coordenadora

**Quem é?**
Sobrecarregada, pragmática e resistente a mudanças complexas.

**Informações Demográficas**
Roberta Lima Costa, 45 anos, é a coordenadora do curso de Bacharelado em Sistemas de Informação (BSI). Acumula funções administrativas, atendimento a alunos e professores, e ainda precisa gerenciar o controle de salas com o apoio da portaria. Sente-se sobrecarregada, é pragmática e demonstra certa resistência a mudanças complexas, pois já tem uma rotina intensa.

**Comportamentos**
Seu comportamento reflete essa sobrecarga: mantém planilhas e registros manuais para controlar horários e disponibilidade. Prioriza soluções simples por falta de tempo para aprender ferramentas complexas. Depende do setor de manutenção, que nem sempre responde com rapidez. Lida com solicitações informais por e-mail, WhatsApp e contato presencial, o que dificulta o rastreamento e a priorização das demandas.

**Necessidades e Objetivos**
Roberta almeja ter uma visão centralizada da disponibilidade das salas, de modo a planejar a alocação com mais segurança. Precisa visualizar e gerenciar todos os chamados de manutenção recebidos, com status atualizado, para poder acompanhar as soluções. Seu objetivo principal é reduzir o volume de solicitações informais e eliminar a dependência de canais dispersos (e-mail, WhatsApp, atendimento presencial), tornando o processo mais organizado e mensurável.

---

**Mapa de Empatia – Roberta (Coordenadora)**
...
| --- | --- | --- | --- | --- | --- |
| Os alunos e professores não têm visibilidade sobre quais salas estão disponíveis, precisando ir à secretaria ou perguntar a terceiros, o que gera incerteza e perda de tempo. | Como aluno ou professor, quero visualizar um calendário com a ocupação de todas as salas, para que eu possa identificar rapidamente os horários livres sem precisar me deslocar. | Marcos / Camila | Alto (afeta a maioria dos alunos e professores) | Baixa (implementação de calendário com consulta ao banco de dados) | **Alta** |
...
| Os coordenadores não possuem uma visão consolidada de todas as reservas, dificultando a aprovação de eventos especiais e a identificação de conflitos. | Como coordenadora, quero visualizar todas as reservas realizadas (com filtros por sala, data e curso) e ter a permissão para aprovar ou excluir reservas conflitantes, para que eu possa gerenciar a ocupação de forma estratégica e garantir a prioridade das atividades acadêmicas. | Roberta | Baixo (apenas coordenadores) | Média (painel com filtros e ações administrativas) | **Média** |
...
| Não há um cadastro centralizado e dinâmico das salas, impossibilitando a adição ou remoção de novos espaços conforme a necessidade da instituição. | Como coordenadora, quero cadastrar, editar e remover salas (com informações como nome, capacidade, localização e recursos), para que o sistema reflita com precisão a infraestrutura real do campus e se adapte a mudanças. | Roberta | Baixo (apenas coordenadores) | Baixa (CRUD básico) | **Alta** *(pré-requisito para que o calendário e as reservas funcionem)* |
| Os usuários não possuem acesso controlado ao sistema conforme seu perfil (aluno, professor, coordenador, manutenção). | Como usuário do sistema, quero realizar login com minhas credenciais institucionais e ter acesso apenas às funcionalidades correspondentes ao meu perfil, para que o sistema seja seguro e cada pessoa veja somente o que é relevante para ela. | Todos | Alto (todos os usuários) | Média (autenticação com controle de papéis/permissões) | **Alta** *(pré-requisito de segurança para todo o sistema)* |
...
- **Como poderíamos** oferecer à coordenadora Roberta uma visão centralizada de todas as reservas e chamados, reduzindo drasticamente o volume de solicitações informais por WhatsApp e e-mail?
...
| --- | --- | --- |
| RF001 | **Visualizar calendário de disponibilidade:** O sistema deve exibir a ocupação das salas em formato de calendário semanal ou mensal, com indicação visual (cores) para salas livres (verde), ocupadas (vermelho) ou com problemas reportados (amarelo). O acesso requer login com matrícula e senha. Usuários que não dispõem de acesso próprio no momento podem dirigir-se à portaria, onde um funcionário autenticado realiza a consulta mediante solicitação. | Marcos / Camila |
...
| RF004 | **Reportar problemas estruturais:** O sistema deve oferecer um formulário com campos para local (sala/laboratório), categoria do problema, descrição detalhada e opção de anexar foto. O envio de chamados requer login com matrícula e senha. Usuários que não disponham de acesso no momento podem registrar o problema presencialmente na portaria, com auxílio de um funcionário. | Camila / Marcos |
| RF005 | **Acompanhar status dos relatos:** O sistema deve exibir ao solicitante uma lista com todos os seus relatos e seus respectivos status (aberto, em andamento, resolvido, em espera). | Camila |
| RF006 | **Gerenciar salas (coordenador):** O sistema deve permitir o cadastro, edição e remoção de salas, com informações como nome, capacidade, localização e recursos disponíveis. *Pré-requisito para o funcionamento do calendário e das reservas.* | Roberta |
| RF007 | **Gerenciar usuários (coordenador):** O sistema deve permitir o cadastro, edição de permissões e remoção de usuários. | Roberta |
| RF008 | **Visualizar e gerenciar todos os problemas (coordenador/manutenção):** O sistema deve exibir uma lista consolidada de todos os problemas reportados, com filtros por local, categoria e status, e permitir a atualização do status de cada solicitação. O histórico por sala deve ser acessível para consulta. | Carlos / Roberta |
| RF009 | **Autenticação e controle de perfis:** O sistema deve permitir login com credenciais institucionais (matrícula e senha) e controlar o acesso às funcionalidades conforme o perfil do usuário (aluno, professor, coordenador, manutenção). Todas as funcionalidades exigem autenticação. A portaria atua como ponto de apoio presencial para usuários que não disponham de acesso próprio no momento, realizando consultas e registros em nome do solicitante mediante identificação por matrícula. | Todos |
...
- **RNF004 – Segurança e autenticação:** Todas as funcionalidades do sistema requerem autenticação com matrícula e senha. Não há acesso público ou anônimo. A portaria serve como canal de atendimento presencial para usuários sem acesso próprio, operando sempre com credenciais de funcionário.
...
**Roque** sugeriu a implementação de um painel visual no estilo Kanban para o módulo de manutenção, onde os problemas reportados pudessem ser organizados por status (Aberto, Em Andamento, Resolvido, Em Espera), facilitando a priorização do serviço.
...
| --- | --- | --- |
| Professores e alunos sofrem com salas ocupadas sem aviso e desejam uma solução. | Professores e alunos podem ter resistência ao cadastro ou ao processo de login. | O sistema exige login para todas as funcionalidades. A portaria serve como ponto de apoio presencial para quem não dispuser de acesso no momento, consultando ou registrando informações em nome do usuário. |
| Problemas de infraestrutura sem registro pioram com o tempo, gerando custo e insatisfação. | Alunos podem não confiar que o problema será resolvido, e podem abandonar o sistema. | Exibir o status do chamado (aberto / em andamento / resolvido / em espera) de forma visível para o solicitante após login. |
| Há demanda real por um canal que feche o ciclo: reporte → análise → resolução → confirmação. | Pode cair em desuso se a equipe de manutenção não responder com agilidade. | Mostrar histórico de chamados resolvidos para demonstrar que o sistema funciona e gera resultados. Gerar relatório para gestão de problemas não solucionados e prazos atrasados. |
...
| Positivos | Negativos | Solução Proposta |
| --- | --- | --- |
| A solução beneficia diretamente a gestão do campus, com potencial de apoio institucional. | Quem será responsável por manter as informações de salas e horários atualizadas no sistema? | A coordenação, em parceria com a portaria, assume a responsabilidade de atualização. O uso das salas fica condicionado à consulta e atualização no sistema. |
...
| O uso de salas vagas pode ser potencializado com um sistema que mostre disponibilidade de forma centralizada. | O sistema pode cair em desuso se não houver atualização constante e resposta ágil da manutenção. | Definir responsáveis claros (coordenação, portaria, manutenção) antes do lançamento do sistema. |
...
A partir das ideias geradas na sessão de brainstorming e da análise pelo Triângulo de Ouro, a equipe priorizou as seguintes funcionalidades para compor o Produto Mínimo Viável (MVP), considerando o equilíbrio entre valor entregue ao usuário e esforço de desenvolvimento:

- **Dashboard inicial:** com atalhos para "Ver salas", "Minhas reservas", "Reportar problema" e "Status dos meus relatos".
- **Calendário de disponibilidade:** visualização semanal das reservas com cores indicativas (verde = livre, vermelho = ocupado, amarelo = com problema). Acessível mediante login. Usuários sem acesso próprio podem consultar a disponibilidade presencialmente na portaria, onde um funcionário autenticado realiza a consulta.
- **Reserva de sala:** seleção de sala, data, horário de início e fim, com validação automática de conflitos. Requer login.
- **Gerenciamento de reservas próprias:** listagem com opções de editar e cancelar reservas (dentro do prazo permitido).
- **Reporte de problemas:** formulário com local, categoria, descrição e anexo de foto (opcional), acessível mediante login. Usuários sem acesso podem registrar o problema presencialmente na portaria, com auxílio de um funcionário.
- **Acompanhamento de relatos:** lista dos problemas reportados pelo usuário com status atualizado (aberto, em andamento, resolvido, em espera).
...
- **Sistema de autenticação:** login com matrícula e senha, obrigatório para todas as funcionalidades. Diferentes níveis de permissão conforme o perfil (aluno, professor, coordenador, manutenção). A portaria atua como canal de atendimento presencial para usuários sem acesso próprio no momento.
...
- **Tela de Calendário de Salas:** exibindo a ocupação semanal com indicação por cores (verde para livre, vermelho para ocupado, amarelo para com problema). Acessível após login.
...
- **Tela de Relato de Problemas:** com campos para local, categoria, descrição e anexo de imagem, além da consulta ao status dos relatos realizados. Acessível após login.
...
1. O usuário acessa a página inicial e realiza login com suas credenciais (matrícula e senha).
2. Após autenticado, é direcionado ao Dashboard, onde visualiza um resumo das próximas reservas e atalhos para as principais ações. Usuários que não dispõem de acesso próprio no momento podem se dirigir à portaria, onde um funcionário autenticado realiza a consulta ou registra o chamado em seu nome.
3. Após autenticado, é direcionado ao Dashboard, onde visualiza um resumo das próximas reservas e atalhos para as principais ações.
4. Ao clicar em "Ver salas", o usuário tem acesso à lista de todos os espaços. Selecionando uma sala, é exibido um calendário semanal com os horários ocupados (em vermelho) e livres (em verde).
5. Para reservar, o usuário clica em um horário livre e é direcionado ao formulário de reserva, onde preenche a data, horário de início e fim, e a finalidade. Ao confirmar, o sistema valida se não há conflitos e registra a reserva.
6. O usuário pode acessar "Minhas reservas" para visualizar todas as suas reservas ativas e, se desejar, editar ou cancelar cada uma delas (desde que respeitado o prazo mínimo de antecedência).
7. Para reportar um problema, o usuário acessa "Reportar problema", preenche o local, a categoria, descreve o defeito e pode anexar uma foto. Ao enviar, o sistema registra o chamado com status "aberto" e exibe uma confirmação de que o chamado foi recebido.
8. Em "Meus relatos", o usuário acompanha a evolução de cada solicitação, visualizando quando o status muda para "em andamento" ou "resolvido".
...
3. Ao selecionar um chamado, o técnico visualiza os detalhes completos (incluindo foto, se houver) e pode alterar o status para "em andamento" ao iniciar o atendimento.
...
**Fluxo da coordenadora:**

1. Após login com perfil de coordenadora, a usuária tem acesso a um menu adicional com opções como "Gerenciar salas", "Gerenciar usuários", "Todos os problemas" e "Todas as reservas".
2. Em "Todos os problemas", a coordenadora visualiza uma tabela com todos os chamados abertos, podendo aplicar filtros por local, categoria ou status.
...
4. Em "Gerenciar salas", a coordenadora pode cadastrar novas salas, editar informações existentes (nome, capacidade, localização) ou remover salas que não estejam mais em uso.
5. Em "Gerenciar usuários", a coordenadora pode cadastrar novos usuários, definir ou alterar seus perfis (aluno, professor, manutenção) e remover usuários inativos.
