...
| Os coordenadores e administradores não possuem uma visão consolidada de todas as reservas, dificultando a aprovação de eventos especiais e a identificação de conflitos. | Como coordenadora ou administradora, quero visualizar todas as reservas realizadas (com filtros por sala, data e curso) e ter a permissão para aprovar ou excluir reservas conflitantes, para que eu possa gerenciar a ocupação de forma estratégica e garantir a prioridade das atividades acadêmicas. | Roberta | Baixo (apenas coordenadores e administradores) | Média (painel com filtros e ações administrativas) | **Média** |
...
| Não há um cadastro centralizado e dinâmico das salas, impossibilitando a adição ou remoção de novos espaços conforme a necessidade da instituição. | Como administradora do sistema, quero cadastrar, editar e remover salas (com informações como nome, capacidade, localização e recursos), para que o sistema reflita com precisão a infraestrutura real do campus e se adapte a mudanças. | Roberta | Baixo (apenas administradores) | Baixa (CRUD básico) | **Alta** *(pré-requisito para que o calendário e as reservas funcionem)* |
...
| RF006 | **Gerenciar salas (administrador):** O sistema deve permitir o cadastro, edição e remoção de salas, com informações como nome, capacidade, localização e recursos disponíveis. *Pré-requisito para o funcionamento do calendário e das reservas.* | Roberta |
| RF007 | **Gerenciar usuários (administrador):** O sistema deve permitir o cadastro, edição de permissões e remoção de usuários. | Roberta |
...
- **Sistema de autenticação:** login com matrícula e senha, obrigatório para todas as funcionalidades. Diferentes níveis de permissão conforme o perfil (aluno, professor, coordenador, administrador, manutenção). A portaria atua como canal de atendimento presencial para usuários sem acesso próprio no momento.
...
**Fluxo da coordenadora/administradora:**

1. Após login com perfil administrativo, a usuária tem acesso a um menu adicional com opções como "Gerenciar salas", "Gerenciar usuários", "Todos os problemas" e "Todas as reservas".
2. Em "Todos os problemas", a administradora visualiza uma tabela com todos os chamados abertos, podendo aplicar filtros por local, categoria ou status.
...
4. Em "Gerenciar salas", a administradora pode cadastrar novas salas, editar informações existentes (nome, capacidade, localização) ou remover salas que não estejam mais em uso.
5. Em "Gerenciar usuários", a administradora pode cadastrar novos usuários, definir ou alterar seus perfis (aluno, professor, coordenador, manutenção) e remover usuários inativos.
